...
upload_iso_to_proxmox() {
  ...
  scp ansible/downloads/windows_10_ISO/windows10.iso $PROXMOX_USER@$PROXMOX_IP:$ISO_DIR
  log "Windows 10 ISO uploaded."

  # Upload Windows Server 2019 ISO
  log "Uploading Windows Server 2019 ISO..."
  scp ansible/downloads/windows_server_2019_ISO/windows_server_2019.iso $PROXMOX_USER@$PROXMOX_IP:$ISO_DIR
  log "Windows Server 2019 ISO uploaded."

  # Upload Snare Central VMA
  log "Uploading Snare Central VMA..."
  scp ansible/downloads/snare_central_ISO/Snare_Central.vma.zst $PROXMOX_USER@$PROXMOX_IP:$DUMP_DIR
  log "Snare Central VMA uploaded."

  # Upload VirtIO ISO
  log "Uploading VirtIO ISO..."
  scp ansible/downloads/virtio-win.iso $PROXMOX_USER@$PROXMOX_IP:$ISO_DIR
